...
#define PosSamListFile "E:\\毕业设计\\代码\\标注文本\\PositiveImageList.txt" //正样本图片的文件名列表
#define NegSamListFile "E:\\毕业设计\\代码\\标注文本\\NegativeImageList.txt" //负样本图片的文件名列表
...
#define TestImageFileName "E:\\毕业设计\\代码\\pos帧待截图\\6.png"  //训练完成后读入一张图片来测试效果
...
void hogsvm()
{
  ...
  /**************读入图片进行HOG行人检测******************/
	...
	Mat src2 = imread(TestImageFileName);
	vector<Rect> found, found_filtered;//矩形框数组
	cout<<"进行多尺度HOG人体检测"<<endl;
	cvtColor(src,src2,CV_RGB2GRAY);
	myHOG.detectMultiScale(src2, found, 0, Size(8,8), Size(32,32), 1.05, 2);//对图片进行多尺度行人检测
  ...
  cout<<"找到的矩形框个数："<<found_filtered.size()<<endl;
	...
	for(int i=0; i<found_filtered.size(); i++)
	{
		...
		rectangle(src, r.tl(), r.br(), Scalar(0,0,255), 5);
	}

	imwrite("ImgProcessed.png",src);
	namedWindow("src",0);
	imshow("src",src);
